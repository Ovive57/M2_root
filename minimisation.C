{
    ...
    double ns = 100; //nombre d'évenements signal
    double ntot = ns+nb; //nombre d'évenements totales
    double k = ns/ntot; //paramètre signal sur bruit. Pour nous 0.009, on cherche s'approcher à ça
    ...
    double amp = 5.0; // amplitude gaussienne (signal)
    ...
    double x; // Pour calculer le x au milieu du bin où on va évaluer la fonction f(x)
    double val_att; // Pour calculer la valeur attendu, i.e. f(x). (lamda)
    double val_mesure; // Pour calculer la valeur mesurée i.e. (n) 
    
    double poisson; // Fonction de poisson
    ...
    for(ki=0;ki<=0.02;ki=ki+0.005){
        ...
        for (bin = 0; bin<=bins-1; bin=bin+1){
            ...
            // Ça c'est juste un print:
            // std::cout<<likelihood[i]<<tot_poisson<<poisson<<std::endl;
